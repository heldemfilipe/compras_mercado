...
export default function MonthlyBars({
  ...
  const labelsAlways = showValues ?? data.length <= 8;
  ...
  return (
    <div className="w-full">
      <div className="relative" style={{ height }}>
        {/* linhas de grade */}
        <div className="pointer-events-none absolute inset-x-0" style={{ top: 0, height: plotH }}>
          {gridVals.map((v, i) => (
            <div
              key={i}
            ...
            >
              <span className="w-9 shrink-0 pr-1 text-right text-[10px] text-ink-faint">
          ...
          {/* média */}
          <div
            className="absolute inset-x-9 border-t border-dashed border-accent/50"
            style={{ top: `${(1 - avg / scaleMax) * 100}%` }}
          >
            <span className="absolute right-0 -top-4 rounded bg-bg px-1 text-[10px] text-accent/80">
              méd {formatBRL(avg)}
            </span>
          </div>
        </div>
          ...
          {data.map((d) => {
            ...
            return (
              ...
              >
                {(labelsAlways || isSel) && (
                  <span
                    className={`mb-1 text-center text-[10px] font-medium tabular-nums ${
                      isSel ? "text-accent" : "text-ink-muted"
                    }`}
                  >
                    {d.total >= 1000
          ...
          {data.map((d) => {
            ...
            return (
              <button
                ...
                disabled={!interactive}
                onClick={() => onToggle?.(d.month)}
                className={`flex-1 pt-1 text-center text-[10px] leading-tight ${
                  isSel ? "font-semibold text-accent" : "text-ink-faint"
                } ${interactive ? "cursor-pointer" : "cursor-default"}`}
              >
